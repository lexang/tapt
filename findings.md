...
## 2026-05-16 TableConvert 页面复看

- 用户指出左中右版本更像后台工作台，不符合轻工具站气质。
- 参考 `https://tableconvert.com/zh-cn/excel-to-sql` 后，新的页面结构改为上中下：
  - 数据源：文件上传和粘贴输入在顶部，保持入口清晰。
  - 在线表格编辑器：表格区域独占中段宽度，避免数据列被挤窄。
  - 表格生成器：转换选项和结果放在底部，符合“生成结果”的阅读顺序。
- 视觉方向从深色后台工作台调整为浅色轻工具站，保留清晰状态标记和紧凑数据表格。
